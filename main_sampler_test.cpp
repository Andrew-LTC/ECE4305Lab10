...
void tappingDetection(SpiCore *spi_p, GpoCore *led_p){
	const uint8_t RD_CMD = 0x0b;
	const uint8_t DATA_REG = 0x08;
	...
	//find magnitudes of x y and z values
	float newMag = abs(x) + abs(y) + abs(z);
	float oldMag = abs(xOld) + abs(yOld) + abs(zOld);
	float magDiff = newMag - oldMag;
	float magMin = 0.04;
	float magMax = 0.7;

	//compare diff in magnitudes
	if(magDiff >= magMin){
		uart.disp("MAG:");
		uart.disp(magDiff);
		uart.disp("\n\r");
		...
		if(magDiff >= magMin && magDiff <= magMax){
			//slope of a line with min MAG of 0.04 and max MAG of 0.7
			int ledAmt = ((15/(magMax - magMin)) * magDiff) - (magDiff * magMin);
			for(int i = ledAmt; i >= 0; i--){
				led_p->write(0x7fff >> (15 - i));
				sleep_ms(25);
				led_p->write(0);
			}
		}
